...
def librtmp_android():
	...
	if download(my_librtmp,"http://anonymous-repo.googlecode.com/svn/trunk/xbmc-tools/librtmp/Android/librtmp.so"):
		...
		subprocess.Popen("su -c 'rm "+os.path.join(librtmp_path, "librtmp.so")+"'")
		subprocess.Popen("su -c 'cp -f "+my_librtmp+" "+librtmp_path+"/'")
		subprocess.Popen("su -c 'chown root.root "+os.path.join(librtmp_path, "librtmp.so")+"'")
		subprocess.Popen("su -c 'chmod 755 "+os.path.join(librtmp_path, "librtmp.so")+"'")
